--[[
    ExtractionSteeringYaxisScan.lua - scan the intitial ion start position and the Y-axis extraction steering and record the ion's displacement from the ion axis in the SEL
]]
...
local ext_y_center = 0     -- voltage for extraction y steering at center of target
local ext_y_range = 300    -- one-direction range of extraction y steering (-ext_y_range, ext_y_range)
local bend_range = 150     -- one-direction range of bender (-bend_range, bend_range)
local nvolts_e = 30        -- number of voltages in extraction steering scan range
local nvolts_b = 15        -- number of voltages in bender scan range
...
file:write("Generated from IonStartLocationSteering.iob\nnumber of ions = "..nions.."\nBender = "..bender_center.."V\nSEL focus = "..sel_focus.."V\nCapture focus = "..capture_focus.."V\nDeceleration Top = "..capture_dec.."V\nYpos,Ext_y,Bender,IonPosition")
  ...
  for i = 1,npoints+1 do
    ...
    PL.reload_fly2('ExtractionSteeringYaxisScan.fly2', {
    ...
    for ext = 1,nvolts_e+1 do
      ext_y = ext_y_center - ext_y_range + (ext-1)*exty_inc
      for bend = 1,nvolts_b+1 do
        bender = bender_center - bend_range + (bend-1)*bend_inc
        -- Set up data recording to file and perform trajectory calculation run.
        data_rec = {ypos,ext_y,bender,nil}
        print('Y pos =', ypos, 'Ext_y =', ext_y, 'Bender =', bender)
        run()
      end
    end
  end
...
function segment.other_actions()
  ...
  then
    data_rec[4] = abs(ion_px_mm - 101.6625)
    rec_pos1 = ion_number
  end
