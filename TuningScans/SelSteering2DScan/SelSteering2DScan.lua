...
local ext_range = 100      -- one-direction range of extraction y,z steering (-ext_*_range, ext_*_range)
local nvolts = 41          -- number of voltages in extraction steering scan range (increment = (2*range)/(n-1)) 26
...
local sel_focus = -1800     -- common voltage on sel for focus
...
local capture_focus = -900  -- focus voltage on capture optics einzel lens
...
file = io.open("data\\ke_20_sel_1800_cfocus_900\\IonStartLocationSteering_ke_20_sel_1800_cfocus_900.csv", "w")
...
function segment.other_actions()
  ...
  -- record ion location at A1 aperture
  if (ion_py_mm >= 354 and rec_pos1 ~= ion_number)
  then
