...
local sel_center = -1800   -- voltage for extraction y,z steering at center of target
local sel_range = 100      -- Range of focus voltages
local nvolts = 20          -- number of voltages in sel focus scan range (increment = (2*range)/(n-1)) 26
local bender_center = 920  -- Bender Voltage
local bender_range = 25   --Bender range
...
bend_inc = (2*bender_range)/(nvolts-1)
...
function segment.flym()
    ...
    for sel = 1,nvolts do
        sel_focus = sel_center - sel_range + (sel-1)*sel_inc
        for bend = -nvolts,nvolts do
          bend_offset = (bend)*bend_inc
          data_rec = {sel_focus,bend_offset,0 , nil,nil}
          run()
        end
...
function segment.fast_adjust()
  ...
  adj_elect10 = bender_center + bend_offset
  adj_elect20 = -1 * bender_center + bend_offset
  adj_elect30 = -1 * bender_center + bend_offset
  adj_elect40 = bender_center + bend_offset
  -- set SEL steering
...
function segment.terminate_run()
    data_rec[3] = ion_count
    data_rec[4] = efficiency/ion_count 
    data_rec[5] = mean(dist_after_deflector)
